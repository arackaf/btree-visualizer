...
export const NonLeafNodeContents: FC<{ node: VisualNode }> = ({ node }) => {
  const hasMultipleKeys = node.data.keys.some((key: any) => Array.isArray(key) && key.length > 1);
  const keysJoined = node.data.keys.map((key: any) => (Array.isArray(key) && key.length > 1 ? `[${key.join(", ")}]` : key)).join(", ");
  ...
  return (
    <>
      ...
      {hasMultipleKeys ? (
        node.data.keys.map((key: any, i: number) => {
